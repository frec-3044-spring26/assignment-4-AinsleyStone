...
lake_ice <- read_excel(`file_name`, sheet = 2)
```
...
plot <- sunapee |>
  ...
  geom_line()+
  theme_bw()+
  labs(title = "Ice of Day of Each Year",
       x = "Year", 
       y = "Day Where ice off occured")
...
I expect the sign of the slope to be negative. This is because if you look at the overall trend of the points, they seem to be moving downward from left to right.
...
The direction of the slope is in fact negative. This matches my expectations from question 2 as the overall trend of the points looked to be going down.
...
The R2 for the regression is 0.2055. This is less than 0.3, indicating that the year and ice_off_doy are not strongly related.
...
The residuals are distributed around 0, but are not normally distributed because there is a left skew.
...
The p-value for the slope is 2.504e-09
...
Based on the slope value (-0.1060227), about 66 years is how long it takes for a 1 week change in ice off day. I think this is meaningful because 0.106 days earlier a year seems like a very small number, but overtime it adds up. A week earlier every 66 years sounds much quicker than 0.106 days a year. This can have large impacts on lake ecosystems.
...
I think I am overall comfortable using this model for additional analysis and prediction. I feel like I understand what the model is and does, and as long as I am given the functions and how to use them I will be able to figure it out. I have been using the ? before functions when I dont remember how to use them and that has been helping me. I also am able to use class() and str() when there is an error to make sure vectors are saved correctly.
...
```{r}
plot + 
  geom_abline(intercept = 318.9486, slope = -0.1060227)
...
**Answer 11:**
The ice-off day of the year in 2040 is 102.6623.
The 90% confidence interval is (-19.43, 14.15).
The 90% confidence interval around the prediction is (83.23, 116,81).
The confidence interval is 33.58 days wide.
...
lower_int <- (-19.43) + 102.66
upper_int <- (14.15) + 102.66
...
According to the linear model, the slope of the plot from before 1970 is -0.1097, and the slope of the plot from after 1970 is -0.3024. In the plot you can visually see that the green line (before 1970), has a less steep slope than the purple line (after 1970). This shows an increase in the rate of ice-off day of year, meaning that overtime the rate at which the ice off day of year is decreasing, is increasing.
...
```{r}
sunapee_before70 <- sunapee |>
  filter(year < 1970)

sunapee_after70 <- sunapee |>
  filter(year >= 1970)

before <- lm(ice_off_doy ~ year, data = sunapee_before70)
after <- lm(ice_off_doy ~ year, data = sunapee_after70)
...
plot2 <- sunapee |>
  ...
  geom_point()+
  theme_bw()+
  labs(title = "Ice of Day Each Year",
       x = "Year",
       y = "Day where Ice off Occured")
